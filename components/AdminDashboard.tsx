...
import { Check, X, Shield, ArrowLeft, RefreshCw, User as UserIcon, Key, Settings, ToggleLeft, ToggleRight, Ban, Trash2, RotateCcw, Users, Calendar, Hash, Activity, AlertTriangle } from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAutoApproveKeys(config.autoApproveApiKeys);
  ...
  const toggleAutoApprove = () => {
    ...
    setSystemConfig({ autoApproveApiKeys: newVal });
  };
                      ...
                      <td className="px-6 py-4 text-right">
                        {user.role !== 'admin' && (
                          <div className="flex items-center justify-end gap-2">
                            {user.status === 'pending' && (<><button onClick={() => handleUserStatusChange(user.id, 'active')} className="p-1.5 bg-green-600/20 text-green-400 hover:bg-green-600 hover:text-white rounded border border-green-600/50 transition-colors"><Check size={16} /></button><button onClick={() => handleUserStatusChange(user.id, 'rejected')} className="p-1.5 bg-red-600/20 text-red-400 hover:bg-red-600 hover:text-white rounded border border-red-600/50 transition-colors"><X size={16} /></button></>)}
                            {user.status === 'active' && (<button onClick={() => handleUserStatusChange(user.id, 'rejected')} className="p-1.5 bg-yellow-600/20 text-yellow-400 hover:bg-yellow-600 hover:text-white rounded border border-yellow-600/50 transition-colors"><Ban size={16} /></button>)}
                            {user.status === 'rejected' && (<button onClick={() => handleUserStatusChange(user.id, 'active')} className="p-1.5 bg-green-600/20 text-green-400 hover:bg-green-600 hover:text-white rounded border border-green-600/50 transition-colors"><RotateCcw size={16} /></button>)}
                            <button onClick={() => openDeleteUserModal(user.id)} className="p-1.5 bg-gray-700/30 text-gray-400 hover:bg-red-600 hover:text-white rounded border border-gray-600/30 hover:border-red-600 transition-colors ml-2"><Trash2 size={16} /></button>
                          </div>
                        )}
                      </td>
                    </tr>
